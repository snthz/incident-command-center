...
- **Attachments** — files on the incident description and on comments (paperclip in the composer), uploaded from the browser straight to a private Supabase Storage bucket and served through short-lived signed URLs; uploads land for other viewers in real time and are recorded in the audit log.
